...
Inside each region, the EKS Auto Mode cluster carries the same internal shape. It runs a set of nodepools — System and General, GPU pools for both x86 and ARM, and a dedicated Inference pool — alongside the services that keep the region honest: a Health Monitor, a Manifest Processor, and an Inference Monitor. Shared storage is EFS, with FSx available when a workload needs it. Because the layout is identical region to region, the same manifest lands the same way whether it runs in us-east-1 or ap-northeast-1, and the accelerator behind it can be an NVIDIA GPU (g4dn, g5, or ARM64 g5g), AWS Trainium, AWS Inferentia, or general-purpose CPU on amd64 or Graviton.
...
The autoscaling target is expressed as part of the deploy, so the endpoint's scaling behaviour lives with its definition in DynamoDB and is reconciled the same way as everything else.
...
3. **Observe** — normalise the tool results into a single Observation.
...
5. **IRSA** — pods use IAM roles for service accounts for their own AWS access, so there are no static credentials sitting inside the cluster.
...
|-------------|---------------|
| NVIDIA GPU | g4dn, g5, and ARM64 g5g — general GPU training and inference. |
